...
| G-11 | コーディングエージェントが Yagra ワークフローを高精度に自律生成・修正できる | スキーマに意味情報（description/examples）が含まれ、バリデーションが修正提案を返し、ハンドラーパラメータが発見可能で、エージェントが生成→検証→修正ループを人間の介入なしに完結できる。加えて、エージェント統合ガイドにより導入手順が明確化されている | Open |
...
- G-11 は G-06（AI-Native 基盤）を深化させるゴール。G-06 では「スキーマ公開・validate CLI・テンプレート」の最低限のインターフェースを整えたが、実際のエージェント自律生成には課題が残る。具体的には、スキーマに意味情報がない（description/examples 未設定）、バリデーションが修正提案を返さない、ハンドラーの params 構造が不透明、stdin 非対応でパイプライン構成が困難、エージェント統合ガイドが不在などの改善を段階的に行う。M-28〜M-34 で実装する。
