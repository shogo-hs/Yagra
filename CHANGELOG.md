...
### Added
- ✨ **Port/Adapter: `LLMProviderPort` に `complete` / `complete_streaming` を追加**: 既存の `complete_structured` に加え、自由記述の text completion (`complete`) と chunk streaming (`complete_streaming`) を port 契約へ正式化。戻り値は SDK 非依存の `LLMCompletion` / `LLMStreamChunk` / `LLMTokenUsage` dataclass（`yagra/ports/outbound/llm_provider.py`）で統一。`LiteLLMProvider` が両メソッドを実装し、`ClaudeAgentSDKProvider` は subset-only（4-field 構造化エラーで `LLMProviderConfigError` を送出）(#28)
- ✨ **Handlers: `create_llm_handler` / `create_structured_llm_handler` / `create_streaming_llm_handler` を `LLMProviderPort` 経由に移行**: handlers 層から litellm への直接依存を撤去し、Port 経由の呼び出しへ統一。Judge handler と同じ Hybrid 解決（明示的な `provider` 引数 > `params["provider"]` 文字列 > default `"litellm"`）に対応。未知の provider 名は `{error, message, summary, hint}` の 4-field 構造化 payload を持つ `LLMHandlerConfigError` を送出 (#28)
- 🧰 **Handler errors モジュール: `yagra.handlers.errors`**: `LLMHandlerError` / `LLMHandlerConfigError` / `LLMHandlerCallError` の正準定義を `yagra/handlers/errors.py` に集約し、`_llm_common` からの循環 import を解消。`yagra.handlers.llm_handler` からの従来 import も re-export でそのまま利用可能（後方互換保証） (#28)
- 📚 **Example: `examples/self-improve/` walking example 追加**: `generate`（LiteLLM / `openai gpt-4o-mini`）→ `judge`（`claude_agent_sdk` / `sonnet`）の 2 ノード構成で LLM-as-a-Judge の自己改善サイクルを体感できる walking example を新設。inline rubric（`clarity` / `accuracy`、scale 1-5）と `judge` handler の default system prompt（rubric から自動生成）を活用し、judge ノードは `prompts.yaml#judge` の user 定義で `generate` の `output_key: "draft"` を参照。stdout で `judge_result.score._overall` を強調表示。`.github/workflows/validate-example.yml` の glob に合致し自動検証対象 (#24)
...
### Changed
- 🔄 **Handlers: Port 経由化に伴うエラーメッセージの統一**: `LLM returned empty response` / `LLM returned None content` を `LiteLLMProvider` 由来の `litellm returned empty choices` / `litellm returned None content` に更新。Provider 層由来のエラーは `LLMProviderCallError` として `llm_retry_loop` で retry され、最終的に `LLMHandlerCallError` にラップされる。`structured_llm_handler` は Port 契約上 token usage を返さないため TraceContext への `record_llm_call` 通知は行わない（`complete` / `complete_streaming` のみ継続サポート） (#28)
- 🔄 **Handlers PARAMS_SCHEMA に `provider` フィールド追加**: `llm` / `structured_llm` / `streaming_llm` の JSON Schema にそれぞれ `provider` プロパティを追加（`enum: ["litellm", "claude_agent_sdk"]`、default `"litellm"`、streaming は `claude_agent_sdk` 非対応のため `enum: ["litellm"]`）。Studio / MCP `list_handlers` / `yagra handlers` から provider 選択肢が発見可能に (#28)
