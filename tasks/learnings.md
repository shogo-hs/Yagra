...
| #24 | **judge handler の default user template は `"{query}"`**。`prompt_ref` / `prompt` 省略時に `state["query"]` を interpolation するが、generate ノード側が `output_key: "draft"` 等で別キーに書き出す walking example では silent に空プロンプト判定となり overall ≈ 1.0 が返る。判定結果を見ないと気付きにくい落とし穴。対処: (a) upstream の `output_key` を `query` に揃える / (b) `prompt_ref` を指定して user template で実 key を参照。`examples/self-improve/` の初版がこれで破綻し、修正後は `prompts.yaml#judge` の user template `"Evaluate the following draft:\n\n{draft}"` で解決 | judge handler を使う全 workflow / 別 handler で「default template が state key をハードコードする」類似パターン |
| #24 | **handler 実装と validator の inline prompt 扱いの非対称**: `create_judge_handler` 内部は `params["prompt"] = {system, user}` を受け付けるが、`application/services/reference_resolver.py:147-155` が validator 段階で inline `prompt` をブロックし `"inline prompt is no longer supported; use prompt_ref to reference an external prompt file"` を返す。workflow YAML 経由では必ず `prompt_ref` を使う必要がある。handler 直接呼出（Python API、pytest）では inline も通るためテストが偽陽性化し得る。両者の差分を docstring / docs に明示しないと agent が誤った YAML を生成する | prompt を扱う全 handler / 新規 handler の docstring に validator 経路の制約も併記するルール |
| #24 | `claude_agent_sdk_provider.py:145` が teardown 時に `RuntimeError: aclose(): asynchronous generator is already running` を stderr に出すが、戻り値と state 更新は正常に完了する pre-existing warning。walking example の実行では無害だが、CI やユーザーが誤解する可能性あり。ThreadPoolExecutor + `asyncio.run()` の async generator クリーンアップ順序が原因。別タスクで対処検討 | claude_agent_sdk provider を使う全 handler / async→sync bridge の teardown 整理タスク |
| #28 | **Port dataclass 設計パターン**: `LLMProviderPort` に `LLMCompletion(content, usage, raw)` / `LLMStreamChunk(delta, done, usage)` / `LLMTokenUsage(prompt_tokens, completion_tokens, total_tokens)` を `@dataclass(frozen=True, slots=True)` / pure Python で追加。`ports/` に具象 SDK を import せず表現力を上げる常套手段。`raw: dict | None = None` を置くことで adapter が provider-native レスポンスを退避でき、将来 debug / trace 目的で復元可能 | 新規 Port method 追加時の戻り値型設計全般 |
| #28 | **循環 import 解消パターン**: `handlers/errors.py` 新設で `_llm_common.py` → `llm_handler.py` の import 矢印を切る。既存 import 経路の backward compat は、移動先が `__all__` でシンボル公開し、移動元は `from yagra.handlers.errors import ... as LLMHandlerCallError` 形式で再 export。`python -c "import yagra.handlers"` のスモークテストで循環消失を確認 | 複数モジュールが共有する error 階層のリファクタ全般 |
| #28 | **Streaming generator priming**: adapter の `complete_streaming` が generator の場合、接続失敗や初期化エラーは `next()` 呼出まで遅延する。handler 側で `iterator = iter(stream); first = next(iterator)` を先行実行することで retry loop 同期化し、`llm_retry_loop` の retry 契約に例外が届くようになる。streaming の retry 信頼性を非 streaming と同水準まで引き上げる | generator を返す全 Port method / async generator の sync 化パターン全般 |
| #28 | **retry 契約の 3 階層整理**: Port 層の例外を handler 層で翻訳する際、`LLMProviderCallError` = retryable（retry loop 対象）、`LLMProviderConfigError` = 非 retryable（即 `LLMHandlerConfigError` 変換）、`LLMHandlerCallError` = 即 re-raise（handler 層で確定した非 retryable）の 3 階層を明確化。どの例外がどこで捕まるか一目で分かり、新しい Port method を追加する際の判断基準になる | retry ポリシーが必要な全 handler / Port 経由の全 RPC |
| #28 | **`complete_structured` の TraceContext 非通知は by design**: Port 契約上 `complete_structured(...) -> dict[str, Any]` は usage を返さないため、handler から TraceContext への `record_llm_call` は呼ばない。将来 dataclass 化（`StructuredLLMCompletion` 等）で usage を返せるようにすれば、`complete` と同じ record path に乗せられる。現在の非対称は設計判断として docs / test で明示しておく | 戻り値型が provider 間で非対称な Port method 全般 / TraceContext 経由の観測可能性設計 |
| #28 | **テスト mock 再編パターン**: handler 内部で SDK 直呼びを撤去した場合、既存 `patch("yagra.handlers.X.litellm")` は adapter 層へ移動する（`patch("yagra.adapters.outbound.llm_providers.litellm_provider.litellm")`）。DI 経路は `FakeXxxProvider(LLMProviderPort)` を用意し patch 不要にする。81 箇所一括書換えの実績あり、handler/adapter 境界の責務整理と同時に行うと効率的 | Hexagonal refactor の mock 再編全般 |
| #28 | **schema に enum で provider の非対応を明示**: `streaming_llm_handler` の PARAMS_SCHEMA に `provider: {"enum": ["litellm"]}` と限定し、`claude_agent_sdk` 非対応を schema レベルで宣言。AI エージェントは `yagra handlers --format json` を見れば試す前に除外できる。全 handler で `["litellm", "claude_agent_sdk"]` の一律 enum にしない方が情報量が高い | 多 provider 対応 handler の schema 設計全般 |
...
| #28 | **Contract v1 → v2 の PM Alignment 往復で複雑度を正しく補正する運用**: #23 と #28 で連続して「PO の初期サイジング M → PM Alignment で L」と補正されている。PM Alignment Agent に `1. 理解確認 / 2. 不明点 / 3. 技術リスク / 4. 代替案 / 5. 見積もり / 6. エスカレーション` の 6 観点を必ず回答させる運用は、コード未読の PO が構造的に見落とす複雑度を補正する harness として機能する | agent-company-v2 の Phase 2b アライメント / 大規模リファクタタスクの初期サイジング補正 |
| #28 | **PMO 指摘 Critical/Major 0 件 4 タスク連続** (#4/#23/#24/#28)、Minor 0 件は 3/4 (#4/#23/#28)。Contract 事前組込（CHANGELOG / pre-commit / 4 フィールドエラー / docstring r""" / backward compat 絶対維持）が完全定着。新タスクの Contract 作成時は「過去 PMO 指摘パターン」を Contract セクションに先回り転記することが標準運用 | 新規タスクの Contract 作成時の標準事項 / ハーネス学習効果の継続計測 |
| #28 | **Port 契約の「対称性と非対称性を許容する」設計哲学**: `complete` / `complete_streaming` は `LLMCompletion` / `LLMStreamChunk` dataclass を返すが、`complete_structured` は `dict[str, Any]` を返し続ける（#23 互換性維持のため）。**今回一括で dataclass 化しないことは Out スコープで明示的に決定**し、backward compat を優先。将来の Should リファクタでまとめて対応する方針を vision-alignment-log に記録し、非対称性を忘れない運用 | 既存 Port の段階的拡張タスク / interface 設計の一貫性 vs 互換性トレードオフ |
