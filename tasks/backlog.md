...
| 2 | `examples/llm-basic/workflow.yaml` を validator 通過する形に修復 | 修正 | Must | - | `yagra validate --workflow examples/llm-basic/workflow.yaml` が error 0 で通過。`version`/`start_at`/`end_at` 補完、エッジは `source:/target:` に統一。README 誘導どおり動作する | pending |
