# CLAUDE.md ヒアリングチェックリスト

## 目的

CLAUDE.md はエージェントの行動境界を固定する「常時参照される仕様書」として扱う。冗長化を避けつつ、迷いやすい判断を先に定義する。
詳細な実行手順はスキル側に寄せ、CLAUDE.md では「何を守るか」と「どのスキルを使うか」を明確にする。
...
- タスク設計ゲートの有無（既定: 必須）
- 進捗管理方法（タスクリスト更新の要否）
- ドキュメント差分確認対象（README/CLAUDE/docs）
...
6. スキル配置方針（再現性）
- 新規開始をスキル同梱テンプレートリポジトリにするか（既定: する）
- 空リポジトリ時のみグローバル bootstrap を初回1回だけ許容するか（既定: 許容する）
- 初回生成後に `<repo>/.claude/skills/<name>/SKILL.md` を正本化するか（既定: 正本化する）
...
- 新規開始はスキル同梱テンプレートリポジトリで進めますか。
- 空リポジトリ時のみ、グローバル `python-project-bootstrap` を初回1回だけ使う運用でよいですか。
- 生成後は `<repo>/.claude/skills/<name>/SKILL.md` を正本に固定してよいですか。
...
- 長いコマンド列は CLAUDE.md ではなくスキルに配置されている。
